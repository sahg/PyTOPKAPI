...
def input_soil(P, Dt, X, ar_Q_to_next_cell, ar_cell_up):
    ...
    -------
    a_s : scalar
        The input flux to the soil store during the current time-step in m^3/s.
...
def output_soil(Vs_t0, Vs_t1_prim, Vsm, a_s, b_s, alpha_s, Dt):
    ...
    transferring volume greater than the saturated soil moisture content to the
    overland store of the same model cell.

    Parameters
    ----------
    Vs_t0 : scalar
        Volume of water in the soil store at the beginning of the current time-
        step (m^3).
    Vs_t1_prim : scalar
        Volume of water in the soil store at the end of the current time-step
        as a result of combined input to and drainage from the non-linear
        reservoir (m^3).
    Vsm : scalar
        Volume of soil moisture for the store under saturated conditions (m^3).
    a_s : scalar
        The input flux to the soil store during the current time-step (m^3/s).
    b_s : scalar
    ...
    -------
    Qs_out : scalar
        Rate of flow out of the soil store during the time-step (m^3/s).
    Vs_out : scalar
        Volume remaining in the soil store at the end of the time-step (m^3).
...
def Qout_computing(V_t0, V_t1_prim, a, Dt):
    ...
    Calculate the mean outflow rate during the current time-step from a generic
    water store. The outflow is calculated as the difference between the inflow
    and rate of change in storage during the time-step.

    Parameters
    ----------
    V_t0 : scalar
        Volume of water at the start of the current time-step (m^3).
    V_t1_prim : scalar
        Volume of water at the end of the current time-step (m^3).
    a : scalar
        The inflow rate to the water store during the time-step (m^3/s).
    Dt : scalar
        The length of the current time-step in seconds.
...
def flow_partitioning(Lambda, Qs_out, Qo_out, W, X, Xc):
    ...
    Calculates the correct partitioning of outflows from the soil and overland
    stores into contributions going to the downstream cell and/or the channel
    store of the current cell (if this exists).
    ...
    Qs_out : scalar
        Outflow from the soil store during the current time-step (m^3/s).
    Qo_out : scalar
        Outflow from the overland store during the current time-step (m^3/s).
    W : scalar
        Width of the channel (m).
    X : scalar
        The lateral dimension of the grid-cell (in m).
    Xc : scalar
        The length of the channel cell, this can be different from `X` if the
        channel runs along the cell diagonal (m).

    Returns
    -------
    Q_to_next_cell : scalar
        Combined outflow from soil and overland to the downstream cell (m^3/s).
    Q_to_channel : scalar
        Combined outflow from soil and overland to the channel store (m^3/s).
    Q_to_channel_sub : scalar
        Outflow from soil store to the channel store (m^3/s).
...
def input_channel(ar_Qc_out, Q_to_channel, ar_cell_up):
    ...
    Calculate the total inflow to the channel as the sum of inflows from
    upstream cells and inflows from the soil and overland stores in the current
    cell.

    Parameters
    ----------
    ar_Qc_out : (N,) Numpy array
        Array of channel outflows from each cell in the catchment for the
        current time-step (m^3/s).
    Q_to_channel : scalar
        Combined outflow from soil and overland to the channel store (m^3/s).
    ar_cell_up : list of `int`
        List of integer indices into `ar_Qc_out`. The indices point to the
        cells upstream of the current cell.
    ...
    a_c : scalar
        ...
        time-step (m^3/s).
    Qc_cell_up : scalar
        The contribution to the total inflow to the channel store from upstream
        cells during the current time-step (m^3/s).
